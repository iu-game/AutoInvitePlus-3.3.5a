...
L["No macro found by that name."] = true
L["Version="] = true
...
L["Random (unpredictable order)"] = true
L["Select Macro Icon"] = true
L["Click an icon to use it for this macro."] = true
L["Filter (e.g. fire, holy, sword)"] = true
...
L["As GS-E is updated, there may be left over macros that no longer relate to sequences.  This will check for these automatically on logout.  Alternatively this check can be run via /gse cleanorphans"] = true
...
L["Incorporate the belt slot into the KeyRelease. This is the equivalent of /use [combat] 6 in a KeyRelease."] = true
...
L["Identity"] = true
L["Version Assignments"] = true
L["Documentation"] = true
-- Configuration-tab tooltip bodies and inline field hints (see
-- GSE_GUI/Editor.lua's attachTooltip/fieldHint calls in GUIDrawMetadataEditor)
L["Which class and spec this macro is written for. Also determines the default archetype used elsewhere in this addon."] = true
L["A short note on the talent build this macro assumes (e.g. a Wowhead link or point spread). Shown under the macro's name in the Sequence Viewer - free text, not read by the game."] = true
L["The macro version used whenever none of the more specific overrides below apply."] = true
L["Version used while inside a 5-player dungeon. Leave on the Default Version to not override it."] = true
L["Version used while inside a Heroic-difficulty dungeon. Leave on the Default Version to not override it."] = true
L["Version used while inside a raid instance. Leave on the Default Version to not override it."] = true
L["Version used for Mythic-difficulty content. Leave on the Default Version to not override it."] = true
L["Version used while in a Battleground or Arena. Leave on the Default Version to not override it."] = true
L["Version used while in a normal party outside a dungeon. Leave on the Default Version to not override it."] = true
L["Free-form notes on what this macro does and how to use it. Shown under its name in the Sequence Viewer."] = true
L["An optional web link with more information. Click it in the Sequence Viewer to open it."] = true
L["Who wrote this macro. For credit/reference only - has no effect on how it runs."] = true
L["Sets the class/spec this macro targets and a short talent-build note for your own reference."] = true
L["Used for any content type left unset below."] = true
L["Which version plays in 5-player content, by difficulty."] = true
L["Which version plays in raid content, by difficulty."] = true
L["Which version plays in PvP, or a plain party outside any instance."] = true
L["Playback Options"] = true
L["Macro Content"] = true
L["Controls the order spells in the Sequence box are cast: Sequential plays them 1, 2, 3... in order; Priority List re-checks from the top each time and casts the first one available; Random picks an available spell unpredictably."] = true
L["Optional cap on how many times an inner /loop block repeats before moving on. Leave blank for unlimited."] = true
L["Permanently deletes this macro version. This cannot be undone."] = true
L["Spells or commands cast once when the key is first pressed, before the main Sequence below. Most macros leave this empty."] = true
L["Commands run once before every cast in the Sequence below, such as target checks. Rarely needed."] = true
L["The macro's main spell rotation - one entry per line, played back in the order set by Step Function above. This is the core of the macro."] = true
L["Spells or commands cast when the key is released, such as trinkets or equipment swaps enabled by the checkboxes below. Most macros leave this empty."] = true
L["Commands run once after every cast in the Sequence above. Rarely needed."] = true
L["Opens the Gnome Sequencer options panel (colours, defaults, and other addon-wide settings)."] = true
L["Sends this macro to another player directly over the addon channel."] = true
L["Saves this macro (all versions) and closes the editor."] = true
L["Permanently deletes this entire macro, all versions included. This cannot be undone."] = true
L["The macro's internal name in Gnome Sequencer. Renaming it here creates a new entry on Save rather than renaming the existing one."] = true
L["Creates a new, empty macro for your current spec and opens it in the editor."] = true
L["Opens the selected macro in the editor. Select a macro from the list above first."] = true
L["Imports a macro from a shared export string."] = true
L["Exports the selected macro as a text string you can share. Select a macro from the list above first."] = true
L["Sends the selected macro to another player directly over the addon channel."] = true
L["Creates a real WoW macro for the selected macro, so it can be dragged onto an action bar. Select a macro from the list above first."] = true
L["Opens the macro recorder, which captures your spell casts in real time to build a sequence automatically."] = true
L["Talents"] = true
L["Help Information"] = true
L["Help Link"] = true
...
L["Delete Icon"] = true
L["Create Icon"] = true
L["Remove Macro Button"] = true
L["Create Macro Button"] = true
L["Deletes the actual WoW macro for this sequence, so it can no longer be dragged onto an action bar. The sequence itself is kept and can have its macro button recreated later."] = true
L["Creates a real WoW macro for this sequence, so it can be dragged onto an action bar."] = true
...
L[" Deleted Macro "] = true
L[" Removed Macro Button for "] = true
...
-- GSE_GUI Editor checkbox tooltips (WotLK port)
L["Reset this macro version's sequence position when you leave combat."] = true
L["Checked: always reset. Unchecked: never reset. Grey (default): use the global 'Reset Macro when out of combat' option."] = true
L["Include this equipment slot in this macro version's KeyRelease line (equivalent to /use [combat] N)."] = true
L["Checked: always include. Unchecked: never include. Grey (default): use this slot's global 'Use ... in KeyRelease' option."] = true
L["Use in KeyRelease"] = true
L["Drag to your action bar to create a macro button."] = true
L["Click to choose a new icon. Drag to your action bar to create a macro button."] = true
