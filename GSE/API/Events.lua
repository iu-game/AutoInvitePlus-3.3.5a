...
function GSE:ADDON_LOADED(event, addon)
  ...
  if addon == GNOME then
    ...
    if not GSEOptions.HideLoginMessage then
      GSE.Print(GSEOptions.AuthorColour .. L["GnomeSequencer-Enhanced loaded.|r  Type "] .. GSEOptions.CommandColour .. L["/gse help|r to get started."], GNOME)
...
local function PrintGnomeHelp()
  ...
  GSE.Print(L["To get started "] .. GSEOptions.CommandColour .. L["/gse|r will list any macros available to your spec.  This will also add any macros available for your current spec to the macro interface."], GNOME)
  GSE.Print(L["The command "] .. GSEOptions.CommandColour .. L["/gse showspec|r will show your current Specialisation and the SPECID needed to tag any existing macros."], GNOME)
...
function GSE:GSSlash(input)
  ...
  elseif string.lower(string.sub(string.lower(input),1,6)) == "export" then
    -- ExportSequence(sequence, sequenceName, compact) expects the actual
    -- sequence TABLE plus its name, not a bare name string (passing just
    -- the name threw "attempt to concatenate a nil value" on sequenceName
    -- every time). Sequence names are also stored UPPERCASE (see
    ...
    if exportSequence then
      ...
      GSE.Print(L["No macro found by that name."])
    end
...
function GSE.prepareTooltipOOCLine(tooltip, OOCEvent, row, oockey)
  ...
  tooltip:SetLineScript(row, "OnMouseUp", function ()
    -- GSE.OOCQueue is a dense array drained via ipairs() in
    -- GSE:ProcessOOCQueue - nil'ing an arbitrary middle index (rather than
    -- removing it) leaves a hole that stops ipairs cold, permanently
