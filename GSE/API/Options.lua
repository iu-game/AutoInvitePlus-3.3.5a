...
--          },
          ...
          deleteOrphanLogout = {
            ...
            desc = L["As GS-E is updated, there may be left over macros that no longer relate to sequences.  This will check for these automatically on logout.  Alternatively this check can be run via /gse cleanorphans"],
            type = "toggle",
            width = "full",
        ...
        args = {
          ...
          titleColour = {
            ...
            set = function(info, r, g, b)
              -- hasAlpha = false means AceConfigDialog's color picker shows
              -- no alpha slider and calls set() with only (info, r, g, b) -
              -- a 5th "a" param here would always be nil, matching every
              -- sibling color option below which hardcodes full alpha (255).
              GSEOptions.TitleColour = string.format("|c%02x%02x%02x%02x", 255 , r*255, g * 255, b*255)
            end,
          },
          authorColour = {
            type = "color",
          ...
          keywordColour = {
            ...
            set = function(info, r, g, b)
              GSEOptions.KEYWORD = GSE.GUISetColour(r, g, b)
            end,
          },
          unknownColour = {
            type = "color",
            ...
            set = function(info, r, g, b)
              GSEOptions.UNKNOWN = GSE.GUISetColour(r, g, b)
            end,
          },
          iconColour = {
            type = "color",
            ...
            set = function(info, r, g, b)
              GSEOptions.CONCAT = GSE.GUISetColour(r, g, b)
            end,
          },
          numberColour = {
            type = "color",
            ...
            set = function(info, r, g, b)
              GSEOptions.NUMBER = GSE.GUISetColour(r, g, b)
            end,
          },
          stringColour = {
            type = "color",
            ...
            set = function(info, r, g, b)
              GSEOptions.STRING = GSE.GUISetColour(r, g, b)
            end,
          },
          conditionalColour = {
            type = "color",
            ...
            set = function(info, r, g, b)
              GSEOptions.COMMENT = GSE.GUISetColour(r, g, b)
            end,
          },
          helpColour = {
            type = "color",
            ...
            set = function(info, r, g, b)
              GSEOptions.INDENT = GSE.GUISetColour(r, g, b)
            end,
          },
          stepColour = {
            type = "color",
            ...
            set = function(info, r, g, b)
              GSEOptions.EQUALS = GSE.GUISetColour(r, g, b)
            end,
          },
          languageColour = {
            type = "color",
            ...
            set = function(info, r, g, b)
              GSEOptions.STANDARDFUNCS = GSE.GUISetColour(r, g, b)
            end,
          },
          shortcutsColour = {
            type = "color",
            ...
            -- Longest label of this tab's colour swatches - the shared
            -- 170px default width (AceConfigDialog-3.0's width_multiplier,
            -- same root cause as the General tab's toggle truncation) clips
            -- it to "Blizzard Functions Col...". Every sibling swatch's
            -- label fits at the default width, so only this one needs
            -- widening rather than converting the whole grid.
            width = "full",
            order = 300,
            hasAlpha = false,
            get = function(info)
            ...
            set = function(info, r, g, b)
              GSEOptions.WOWSHORTCUTS = GSE.GUISetColour(r, g, b)
            end,
          },
        },
      },
  ...
  for k,v in pairs(GSEOptions.AddInPacks) do
    ...
    OptionsTable.args.pluginsTab.args[v.Name] = {
      ...
      -- there's no guaranteed single-column flow for an arbitrary, data-
      -- driven number of entries (one per registered AddInPack) - same root
      -- cause class as the General/Colour tabs' truncation, just collision
      -- instead of clipping. width="full" guarantees each plugin gets its
      -- own row regardless of how many are registered.
      width = "full",
      func = function(info, val)
