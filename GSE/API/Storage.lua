  ...
  if found then
    if GSE.isEmpty(GSELibrary[classid][sequenceName].ManualIntervention) then --- Added by me
		...
		if sequence.MacroVersions and GSELibrary[classid] and GSELibrary[classid][sequenceName] and GSELibrary[classid][sequenceName].MacroVersions then
		  for k,v in ipairs(sequence.MacroVersions) do
		    -- Check ALL existing versions for a match before deciding v is
		    -- new - the old code inserted v once per NON-matching existing
		    -- version (instead of once, after confirming no match exists
		    -- among any of them), which duplicated v N-1 times for N
		    -- existing versions, and did so by table.insert-ing into the
		    -- very table this inner ipairs() loop was iterating.
		    local existingVersions = GSELibrary[classid][sequenceName].MacroVersions
		    local alreadyExists = false
		    for i, j in ipairs(existingVersions) do
			...
			table.insert(existingVersions, v)
...
function GSE.CreateMacroIcon(sequenceName, icon, forceglobalstub)
  ...
  if sequenceIndex > 0 then
    ...
    elseif numAccountMacros >= MAX_ACCOUNT_MACROS - 1 and GSEOptions.overflowPersonalMacros then
     ...
     -- `icon` was silently ignored here (hardcoded to 1, WoW's default
     -- question-mark icon) despite every caller already resolving the
     -- sequence's real chosen icon before calling this function - the
     -- Sequence.Icon pipeline was wired correctly end to end except for
     -- this one hardcoded value. GSE.ResolveMacroIconIndex additionally
     -- normalizes the bare-texture-name-string shape some sequences' Icon
     -- carries (imported/seed data) to the numeric index CreateMacro
     -- actually requires - passing a string through threw "Usage:
     -- CreateMacro(name, iconIndex, body, perCharacter)" for any such
     -- sequence.
     local sequenceid = CreateMacro(sequenceName, GSE.ResolveMacroIconIndex(icon), GSE.CreateMacroString(sequenceName), (forceglobalstub and false or GSE.SetMacroLocation()) )
	end
  end
end
...
function GSE.PrepareLogout(deletenonlocalmacros)
  ...
  -- CleanOrphanSequences ultimately calls the protected DeleteMacro() API
  -- (via DeleteMacroStub). PLAYER_LOGOUT (this function's only caller) fires
  -- on every /reload as well as a real logout, including one mid-combat -
  -- calling a protected function from that event handler while in combat
  -- fails with "Interface action failed because of an AddOn" and leaves a
  -- misleading "Deleted Orphaned Macro" message printed regardless (the
  -- print happens before the call, not after checking it succeeded).
  -- Skipping while in combat is safe: this is background maintenance, not
  -- something that needs to happen on every single reload.
  if GSEOptions.deleteOrphansOnLogout and not InCombatLockdown() then
...
--- This runs from PLAYER_LOGOUT (see PrepareLogout in this file), which
--- fires on every /reload as well as a real logout - a UI-teardown moment,
--- not a stable point to trust class detection. GetCurrentClassID() derives
--- the class ID from a fuzzy UnitClass("player") name match rather than a
--- fixed value; if that lookup ever comes back empty/wrong here, every
--- sequence belonging to the character's REAL class - stored under its
--- real class ID, not under GSELibrary[0] - would wrongly look "not found"
--- and get bulk-deleted as "orphaned", exactly matching a live report of a
--- real, currently-open macro ("UDK") being deleted on repeated /reloads.
--- Checking every class's table (not just current + 0) fixes that
--- regardless of the exact trigger, and is strictly more permissive than
--- before - it can only prevent incorrect deletions, never cause new ones.
  ...
  for macid = 1, maxmacros do
    ...
    if not GSE.isEmpty(mname) then
      for classid, library in pairs(GSELibrary) do
        if not GSE.isEmpty(library[mname]) then
          found = true
          break
        end
      end
...
function GSE.OOCCheckMacroCreated(SequenceName, create)
  ...
  if macroIndex and macroIndex ~= 0 then
    ...
    if create then
      -- Guarded the same way as the found-branch above: classid here is
      -- resolved independently (line 856/858) from the entry's own real
      -- classid, so GSELibrary[classid][SequenceName] can be nil (e.g. a
      -- cross-class sequence import) - indexing .Icon on it directly would
      -- throw "attempt to index a nil value".
      -- No Statics.QuestionMark fallback here: it's a texture NAME string
      -- ("INV_MISC_QUESTIONMARK", correct for display-only uses like
      -- GSE.GetMacroIcon's SetImage calls), but CreateMacroIcon ultimately
      -- passes this straight into the native CreateMacro(name, iconIndex,
      -- ...) API, which requires a NUMERIC index - passing that string
      -- through as a truthy value silently skipped CreateMacroIcon's own
      -- `icon or 1` numeric fallback and threw "Usage: CreateMacro(name,
...
--- This removes a macro Stub. `message` lets callers that are removing a
--- macro on purpose (deleting its sequence, or the Viewer's "Remove Macro
--- Button") say so accurately - the default "Deleted Orphaned Macro"
--- wording is only correct for CleanOrphanSequences' own background
--- cleanup, where the macro really has no sequence behind it any more.
--- Reusing that wording for an intentional removal reads as if something
--- went wrong when nothing did.
function GSE.DeleteMacroStub(sequenceName, message)
  ...
  if mname == sequenceName and mbody then
    ...
    if string.lower(trimmedmbody) == string.lower(trimmedcompar) then
      GSE.Print((message or L[" Deleted Orphaned Macro "]) .. mname, GNOME)
...
--- Sequence.Icon is stored inconsistently across the codebase - a numeric
--- macro-icon index when set via the Editor's icon picker (GUIShowIconPicker
--- passes GetMacroIconInfo's own loop index straight through), but a bare
--- texture-name STRING (e.g. "Spell_Frost_FrostNova") for sequences whose
--- icon came from imported/seed data instead. GSE.GetMacroIcon above can
--- return either straight to an AceGUI Icon widget's SetImage, which accepts
--- both - but the native CreateMacro/EditMacro APIs require a real numeric
--- iconIndex and throw "Usage: CreateMacro(name, iconIndex, body,
