...
function GSE.TranslateSequenceFromTo(sequence, fromLocale, toLocale, sequenceName)
  ...
  if GSE.isEmpty(GSE.TranslatorLanguageTables[Statics.TranslationKey][toLocale]) then
    GSE.Print(L["Target language "] .. toLocale .. L[" is not available.  Unable to translate sequence "] ..  sequenceName)
    return sequence
  end
