...
--- <code>GSStaticRandom</code> is a static step function that picks a random
--    ability from the sequence on every press, instead of stepping through
--    in order.
Statics.RandomImplementation = [[
  step = math.random(1, #macros)
]]

--- Same as GSStaticRandom but confined to the macro's inner-loop range
--    (loopstart/loopstop), for when Inner Loop Limit is set alongside
--    Random - otherwise a random pick could land outside the loop and never
--    return to it.
Statics.LoopRandomImplementation = [[
  step = math.random(loopstart, (loopstop and loopstop > 0) and loopstop or #macros)
]]
