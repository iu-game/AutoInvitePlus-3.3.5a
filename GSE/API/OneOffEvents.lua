...
function GSE.Update2601()
    if GSE.isEmpty(GSEOptions.Update2601) then
        ...
        -- Seed a sane starting size only once, the first time this
        -- migration runs. These two lines used to sit outside this guard
        -- and ran on every single login/reload, silently discarding
        -- whatever size the user had resized the Sequence Editor to (see
        -- GSE_GUI/Editor.lua's OnSizeChanged, which persists live resizes
        -- into these same two options).
